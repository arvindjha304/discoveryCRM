...
//	public $headerFileName = 'header.xls';
		...
		foreach ($rows as $row) {
			...
			foreach ($cells as $cell) {
				...
				if (isset($this->coll_options[$k][trim((string) $cell)]))
					$cell_p['text'] = str_replace('Â' , '' , $this->strip($this->coll_options[$k][trim((string) $cell)]));
				else
					$cell_p['text'] = str_replace('Â' , '' , $this->strip(trim((string) $cell)));
	...
	public function printGridExcel() {
		...
		if(isset($this->reportExportType)){
			if($this->reportExportType=='ReportBuilder')
				$this->wrapper->outXLS($this->title,$this->outputType,$this->reportExportType,$this->exportedFileName,$this->userid,$this->compid);
			else
				$this->wrapper->outXLS($this->title,$this->outputType,$this->reportExportType,$this->exportedFileName,'',$this->compid,$this->fileLocation);
		}
		else
		{
