...
if(isset($compid)){
	require_once FILE_PATH.'/dhtmlxgrid/codebase/grid-excel-php/lib/PHPExcel.php';
	require_once FILE_PATH.'/dhtmlxgrid/codebase/grid-excel-php/lib/PHPExcel/IOFactory.php';
}else{
...
class gridExcelWrapper {
	...
	public function outXLS($title, $type = 'Excel2007',$reportExportType='',$exportedFileName='',$userid='',$compid='',$fileLocation='',$filename='') {
		...
		switch (strtolower($type)) {
			...
			default:
				...
				if($reportExportType=='ReportBuilder'){
					$baseUrl = Zend_Controller_Front::getInstance()->getBaseUrl();
					$dirName = $_SERVER['DOCUMENT_ROOT'].$baseUrl.'/reportExportFiles/'.$compid;
					if (!file_exists($dirName)) mkdir($dirName, 0777, true);
					$objWriter->save($dirName.'/'.$exportedFileName); 
				}elseif($reportExportType=='CompanyReport'){
